...
def test_alignn_featurizer():
    ...
    try:
        ...
        if featurizer.is_alignn_active():
            print("✅ Using ALIGNN embeddings")
            try:
                preds = featurizer.predict(X_small)
                mae = float(np.mean(np.abs(preds - y_train.values[: len(preds)])))
                print(f"   ALIGNN preview MAE on sample: {mae:.4f} eV/atom")
            except Exception as pred_err:
                print(f"⚠️  Prediction preview failed: {pred_err}")
        else:
